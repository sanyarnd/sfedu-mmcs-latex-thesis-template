...
В случае отсутствия указанных выше шрифтов происходит fallback на `Computer Modern` (актуально для `Linux`/`Mac`) и `TeX Gyre Termes Math`. При необходимости используемые шрифты можно задать в `.cls` файле.

_(Буду благодарен, если кто-нибудь подберет наилучшее сочетание альтернативных шрифтов, доступных на большинстве платформ и сделает pull request)_
